...
def main(config_paths):
    ...
    print(data.classes)
    network = LSTM_fixed_len(embedding_dim=30, hidden_dim=256, classes=30)
    train_set, valid_set = dataloaders.loader.split_random(data, frac=0.7)

    config = trainer.TrainConfig(train_set=train_set, validation_set=valid_set, batch_size=16, epochs=10, lr=0.0001, momentum=0.01, criterion=nn.CrossEntropyLoss(), seed=None)
